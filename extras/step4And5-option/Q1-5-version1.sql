...
SELECt aa.nom,aa.prenom,gg.genre_nom,count(*) borrowings_per_genre 
FROM emprunte ee, livre ll, adherent aa ,genres gg
where ee.livre_id = ll.livre_id and aa.no_adherent = ee.no_adherent and ll.genre = gg.genre_id
group by aa.no_adherent,gg.genre_id
ORDER BY count(*) DESC

/* Q5. Number of borrowings for each book for each adherant? (It mean ech book has borrowed 
